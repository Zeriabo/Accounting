...
{ 
    var incomesheet,resultIncomesheet={} 
    ...
    incomesheet={name:name,mvalue:debitValue,cvalue:creditValue,res:result}
...
// for loop to walk threw the balance array and calculate the sum or debits and the sum of credits
    ...
    resultIncomesheet={ name:"Result",mvalue:debitValue,cvalue : creditValue,res:(debitValue>creditValue) ? 
...
export class IncomeStatement extends Component {  
    ...
    debitArr:[],creditArr:[], incomestat:null,trailstat:null,creditsum:null,debitsum:null,value:0,
   ...
   getBalance=  await axios.get('http://localhost:4000/getBalance', {
    ...
    .then((response) => {
       ...
       setTimeout(() => 
       {
        ...
        setTimeout(() => {this.setState({kind: null})}, 2000)
        setTimeout(()=> {this.setState({incomestat:getSheet})},1999) 
        setTimeout(()=>{ NotificationManager.success('Success message', 'Data has been retrieved!', 3000);},1900)
       })
    .catch((err) => { 
...
resetstate(e)
{
    ...
    setTimeout(()=>{this.setState({incomestat:null});},1000)
...
//Posting the data using Fetch
    return fetch('https://fullstack-accounting-backend.herokuapp.com/saveIncomedata', {
...
Displayincomestatement = (props) => {
 ...
 if(this.state.incomestat){  return ( 
...
render() { 
    ...
    <tr>
      <th scope="col" colSpan='4'><font  size = '4' color='#2e20a4'>Fill the Income Sheet</font></th>
    </tr>  
</thead>
...
<tbody>
     ...
     <tr>
      <td> <label htmlFor="exampleFormControlInput1"><font color='#2e20a4'><select className="form-control" id="select1" onChange={this.handleDebitAcc.bind(this)}>
              ...
              { this.AssetLiabilityAccounts.map(acc =>  <option key={acc.id}> {acc.name}</option>)}
              </optgroup>
               <optgroup label = "Revenue">    
              { this.RevenueAccounts.map(acc =>  <option key={acc.id}> {acc.name}</option>)}
              </optgroup>
              <optgroup label = "Expenses">    
              { this.ExpenseAccounts.map(acc =>  <option key={acc.id}> {acc.name}</option>)}
              </optgroup>
...
<tr  className="table-info"><th scope="col" colSpan='4'><font  size = '4' color='#2e20a4'>View Trail and Income Sheet</font></th></tr>
 ...
 <td><button type="submit" onClick={this.activateBalance.bind(this)}   className="btn btn-outline-info"> View Income Sheet</button></td></tr>
